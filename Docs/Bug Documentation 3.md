...
- Problem: Slot occupancy/UI wiring—`Slot.IsOccupied` is inverted (returns true when `_itemInSlot == null`), causing empty slots to be treated as occupied and NREs when dereferencing `GetItemInSlot` (`Assets/Scripts/InventoryScripts/Slot.cs:22-27`). `Slot.AddItem` never subscribes to `itemToken.onAmountChanged`, so stack text never updates; `ChangeSlotUIData` sizes the icon/text to raw item grid size instead of slot-size-scaled; and `InventoryUIController.InitSlot` swaps x/y when initializing slots and never instantiates the static dictionary (`Assets/Scripts/InventoryScripts/Slot.cs:57-106`, `Assets/Scripts/InventoryScripts/InventoryUIController.cs:22-67`). Merge/overflow handling is missing, so partial stacks aren’t split and zero-count items aren’t removed.
...
- Action: Fixed `IsOccupied` to reflect actual occupancy, added stack change subscription/unsubscription when adding/removing items so UI updates correctly, and scaled slot visuals by slot pixel size (`Assets/Scripts/InventoryScripts/Slot.cs:22-27,57-106`). Set `_slotSize` to `32x32` in the scene `InventoryUIController` to match the grid cell size.
